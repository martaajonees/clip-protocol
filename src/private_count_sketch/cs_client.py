...
from utils.utils import load_dataset, generate_error_table, generate_hash_functions, generate_hash_function_G
...
class CSClient:
    ...
    def estimate_client(self,d):
        ...
        for i in range(self.k):
            selected_hash = self.H[i]
            min_estimation = min(min_estimation, self.M[i,selected_hash(d)] * self.G[i](d))
